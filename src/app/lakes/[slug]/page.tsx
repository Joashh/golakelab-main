...
async function getLakeSections(lakeId: number) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_WP_URL}/wp-json/wp/v2/lake-section?_embed`,
    { cache: "no-store" }
  );
...
export default async function Page({ params }: Params) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto  -mt-10 relative z-10 grid grid-cols-2 md:grid-cols-4 gap-4 bg-white rounded-2xl shadow p-5 border border-gray-100">

        {distance && (
          <div className="flex flex-col items-center justify-center gap-2 md:border-r md:border-gray-200 md:pr-6">
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#d0e8e8]">
              <FiMapPin className="text-[#09637e]" />
            </div>
            <div className="text-center leading-tight">
              <p className="text-xs text-gray-500">Distance</p>
              <p className="font-semibold text-[#0F766E]">{distance}</p>
            </div>
          </div>
        )}
        ...
        {depth && (
          <div className="flex flex-col items-center justify-center gap-2 md:border-r md:border-gray-200 md:pr-6">
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#d0e8e8]">
              <FiTrendingDown className="text-[#09637e]" />
            </div>
            <div className="text-center leading-tight">
              <p className="text-xs text-gray-500">Max Depth</p>
              <p className="font-semibold text-[#0F766E]">{depth}</p>
            </div>
          </div>
        )}
        ...
        {area && (
          <div className="flex flex-col items-center justify-center gap-2 md:border-r md:border-gray-200 md:pr-6">
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#d0e8e8]">
              <FiLayers className="text-[#09637e]" />
            </div>
            <div className="text-center leading-tight">
              <p className="text-xs text-gray-500">Surface Area</p>
              <p className="font-semibold text-[#0F766E]">{area}</p>
            </div>
          </div>
        )}
        ...
        {fish && (
          <div className="flex flex-col items-center justify-center gap-2">
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#d0e8e8]">
              <IoFishOutline  className="text-[#09637e]" />
            </div>
            <div className="text-center leading-tight">
              <p className="text-xs text-gray-500">Fish Type</p>
              <p className="font-semibold text-[#0F766E]">{fish}</p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-shrink-0">
                <div className="w-full md:w-48 h-48 rounded-xl overflow-hidden border border-gray-200 shadow-sm">
                  <img
                    src={image}
              ...
              <div className="flex-1 space-y-4">
                ...
                {/* NAME + LOCATION */}
                <div>
                  <h3 className="text-2xl font-semibold text-[#0F766E]">
                    {lake.title.rendered}
                  </h3>
